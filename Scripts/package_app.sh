...
APP_NAME=${APP_NAME:-NotchBrow}
BUNDLE_ID=${BUNDLE_ID:-app.notchbrow.browser}
MACOS_MIN_VERSION=${MACOS_MIN_VERSION:-13.0}
MENU_BAR_APP=${MENU_BAR_APP:-1}
SIGNING_MODE=${SIGNING_MODE:-}
APP_IDENTITY=${APP_IDENTITY:-}
source "$ROOT/updates.env"
...
for ARCH in "${ARCH_LIST[@]}"; do
  swift build --product NotchBrow -c "$CONF" --arch "$ARCH" -Xswiftc -Osize
done
...
<dict>
    ...
    <key>UpdateRepository</key><string>${UPDATE_REPOSITORY}</string>
    <key>UpdatePublicKey</key><string>${UPDATE_PUBLIC_KEY}</string>
</dict>
</plist>
PLIST
...
print(f"App size: {size:,} bytes ({size / 1_000_000:.2f} MB)")
if size > 2_000_000:
    raise SystemExit("App exceeds the 2 MB size budget")
PY_SIZE
